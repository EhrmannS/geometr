...
vis_old <- function(..., layer = NULL, window = NULL, theme = gtTheme, trace = FALSE, image = FALSE,
                      new = TRUE, clip = TRUE){

  # layer = NULL; window = NULL; theme = gtTheme; trace = FALSE; image = FALSE; new = TRUE; clip = TRUE
  # library(checkmate); library(grid); library(rlang)
  # objs <- list(random)
  ...
  end_time <- Sys.time()
  timings <- bind_rows(timings, tibble(activity = "prepare objects", duration = end_time - start_time))
  ...
  end_time <- Sys.time()
  timings <- bind_rows(timings, tibble(activity = "start plot", duration = end_time - start_time))
  ...
  for(i in 1:panels){
    ...
    if(newPlot | (!newPlot & obj$type == "raster")){
      ...
      end_time <- Sys.time()
      timings <- bind_rows(timings, tibble(activity = "prepare panel", duration = end_time - start_time))
      ...
      end_time <- Sys.time()
      timings <- bind_rows(timings, tibble(activity = "plot title", duration = end_time - start_time))
      ...
      end_time <- Sys.time()
      timings <- bind_rows(timings, tibble(activity = "plot yaxis", duration = end_time - start_time))
      ...
      end_time <- Sys.time()
      timings <- bind_rows(timings, tibble(activity = "plot xaxis", duration = end_time - start_time))
      ...
      end_time <- Sys.time()
      timings <- bind_rows(timings, tibble(activity = "plot box", duration = end_time - start_time))
      ...
      end_time <- Sys.time()
      timings <- bind_rows(timings, tibble(activity = "plot grid", duration = end_time - start_time))
