...
#'
#' An \code{geoTheme} stores a theme to \code{\link{visualise}} vector and gridded
#' objects. It is recommended to use \code{\link{setTheme}} to modify a
#' \code{geoTheme}, because it carries out all the checks and makes sure that
#' names of the parameters are properly matched.
#' @slot title [named list(3)][list]\cr properties of the title.
#' @slot box [named list(5)][list]\cr properties of the bounding box.
#' @slot xAxis [named list(5)][list]\cr properties of the x-axis, its labels
#'   and ticks.
#' @slot yAxis [named list(5)][list]\cr properties of the y-axis, its labels
#'   and ticks.
#' @slot grid [named list(5)][list]\cr properties of the major and minor grid.
#' @slot legend [named list(9)][list]\cr properties of the legend, its title,
#'   labels, ticks and bounding box.
#' @slot scale [named list(6)][list]\cr properties of scaling parameters to
#'   attributes/variables.
#' @slot parameters [named list(7)][list]\cr parameters of the plot object.

themeClass <- setClass(Class = "geoTheme",
                       slots = c(title = "list",
                                 box = "list",
                                 xAxis = "list",
                                 yAxis = "list",
...
#' Print geoTheme in the console
#'
#' @param object [geometr theme][geoTheme]\cr object to \code{show}.
            ...
            cat(ifelse(object@legend$plot,
                       paste0(green('\u2714'), yellow(" legend   "), " with values ordered ", cyan(ifelse(object@legend$ascending, "ascending", "descending")), " in ", cyan(object@legend$bins), " bins and a relative height of ", cyan(object@legend$yRatio), "\n",
                              ifelse(object@legend$label$plot,
                                     paste0(green('\u2714'), yellow("  - label  "), "in ", cyan(object@legend$label$colour), " with fontsize ", cyan(object@legend$label$fontsize), " rounded to ", cyan(object@legend$digits), ifelse(object@legend$digits == 1, " digit", " digits")),
                                     paste0(red('\u2716'), yellow("  - label  "))), "\n",
