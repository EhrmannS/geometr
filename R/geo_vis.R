...
#' @param window [data.frame(2)][data.frame]\cr two opposing corners of a rectangle
#'   to which the plot is limited.
#' @param theme [geometr theme][geoTheme]\cr the theme from which to take graphical
#'   parameters; see \code{\link{geo_theme}} for details.
...
#'   (\code{FALSE}, default).
#' @param new [logical(1)][logical]\cr force a new plot (\code{TRUE}, default).
#' @param clip [logical(1)][logical]\cr clip the plot by the plot box
...
#'
#' @importFrom checkmate assertClass assertLogical assertDataTable
#' @importFrom geomio getLayers getNames getType getWindow getProvenance
...
geo_vis <- function(..., window = NULL, trace = FALSE, new = TRUE, clip = TRUE,
                    theme = geoTheme){

  # library(geometr); library(checkmate); library(grid); library(rlang); library(tibble); library(dplyr); library(purrr)
  # window = NULL; theme = geoTheme; trace = FALSE; new = T; clip = FALSE;
  # objs <- list(gc_raster(gtGeoms$grid$continuous)); plotParams <- list(linecol = "fid")
  # Rcpp::sourceCpp('src/unique.cpp')

  # check arguments ----
  assertDataFrame(x = window, any.missing = FALSE, nrows = 2, ncols = 2, types = "double", null.ok = TRUE)
  assertClass(x = theme, classes = "geoTheme", null.ok = TRUE)
  ...
  if(length(objs) != 0){
    ...
    for(i in seq_along(objs)){
      ...
      if(layerNames[i] %in% c("colours", "missingcol", "linecol", "fillcol", "linetype", "linewidth", "pointsize", "pointsymbol")){
        ...
        objsName <- geomio::getNames(x = eval_tidy(expr = objs[[i]]))
    ...
    plotObjects <- list(sketching = geo_point(window = tempWindow, vertices = 0))
  }
  ...
  for(i in seq_along(plotObjects)){
    ...
    if(trace){
      ...
      theHist <- getProvenance(x = theObject)
...
#' @param from [geometr theme][geoTheme]\cr a geoTheme object to modify.
#' @param title [named list(3)][list]\cr \itemize{
...
#' (myTheme <- geo_theme(title = list(plot = FALSE)))
#'
#' geo_vis(input, theme = myTheme)
#' @importFrom checkmate assertList assertLogical assertNames
#' @export

geo_theme <- function(from = NULL, title = NULL, box = NULL, xAxis = NULL,
                      yAxis = NULL, grid = NULL, legend = NULL, scale = NULL,
                      parameters = NULL){

  assertClass(x = from, classes = "geoTheme", null.ok = TRUE)
  if(is.null(from)){
    from <- geoTheme
  }
  out <- from
