...
.makePlot <- function(x, window, image, theme = gtTheme, ...){
  ...
  # modify the theme according to 'newParams' ----
  theTheme <- .updateTheme(x = x, newParams = newParams, theme = theme)

  # make the grob ----
  theGrob <- .makeGrob(x = x, theme = theTheme)
  out$grob <- theGrob

  # make the legend ----
  theLegend <- .makeLegend(x = x, theme = theTheme)
