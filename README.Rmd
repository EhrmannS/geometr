...
Think of a set of polygons that has individual attributes at the `POLYGON` level, but also attributes at the group (`MULTIPOLYGON`) level, i.e., where all polygons of that set have the same value of the respective attributes.

make up a `MULTIPOLYGON` with attributes that are valid for the overall group only.
...
You can use quick options to scale the plot parameters to the arguments of the `geom` to modify which aspects of and object should be shown in the plot, for example when scaling `fillcol` to the attribute `NWBIR74`.
