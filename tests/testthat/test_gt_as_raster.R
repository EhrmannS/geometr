...
test_that("output has class RasterLayer", {
  ...
  aRaster <- gt_as_raster(geom = aGeom, res = c(0.1, 0.1))
  expect_class(aRaster, "RasterLayer")
})
...
test_that("Error if arguments have wrong value", {
  ...
  expect_error(gt_as_raster(geom = "bla"))
  expect_error(gt_as_raster(geom = notAGeom))
  expect_error(gt_as_raster(geom = aGeom, negative = "bla"))
  expect_error(gt_as_raster(geom = aGeom, crs = "LAEA"))
})
