...
test_that("transform geom to LINESTRING sf", {
  ...
  # test with feature attributes
  lineGeomF <- setTable(x = lineGeom, slot = "feat",
                        table = data.frame(gid = 1, feature = "a"))
...
test_that("transform geom to MULTILINE sf", {
  input <- gtGeoms$line
  input@feat$gid <- c(1, 1, 2)
  input@group <- tibble(gid = c(1, 2))
  lineGeomF <- setTable(x = input, slot = "feat",
...
test_that("transform geom to POLYGON sf", {
  ...
  # test with feature attributes
  polyGeomF <- setTable(x = polyGeom, slot = "feat",
