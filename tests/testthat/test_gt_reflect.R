...
test_that("output is valid geometry", {
  ...
  output <- gt_reflect(obj = input, angle = 30)
  ...
  window <- data.frame(x = c(0, 80),
                       y = c(0, 80))
  input <- gs_polygon(anchor = coords, window = window)
  output <- gt_reflect(obj = input, angle = -45, fid = 1)
  expect_class(output, classes = "geom")
  expect_true(output@type == "polygon")
  ...
  window <- data.frame(x = c(0, 80),
                       y = c(0, 80))
  input <- gs_polygon(anchor = coords, window = window)
  output <- gt_reflect(obj = input, angle = c(30, -45))
  expect_class(output, classes = "geom")
  expect_true(output@type == "polygon")
  expect_data_frame(output@point, any.missing = FALSE, nrows = 9, ncols = 3)
})
...
test_that("Error if arguments have wrong value", {
  ...
  expect_error(gt_reflect(obj = "bla"))
  expect_error(gt_reflect(obj = input, angle = "bla"))
  expect_error(gt_reflect(obj = input, fid = "bla"))
  expect_error(gt_reflect(obj = notinput))
})