...
This slot is filled with information by the function `gc_geom()`, by all `gs_*()` functions and by all `gt_*` functions.
In case you want to make use of this slot, it can be set via `setHistory()`.

```{r}
visualise(aPoly)

aLine <- gs_line(anchor = aPoly) %>% 
  gt_reflect(angle = 45)
visualise(aLine, linecol = "deeppink", new = FALSE, trace = TRUE)
```

# Casting

The previous example shows casting in `geometr`.
