...
`nc_sf` contains of 100 "`MULTIPOLYGONS`" where in fact only a small set of them are composed of several polygons.
This is due the definition of simple features, that one "simple" feature can be of a *multi\** type that comprises several closed paths in the same object and which would thus be called `MULTIPOLYGON`; yet, a multipolygon is not defined so that it than also *has to* contain several features but could as well contain only a single polygon.
In `geometr` these inconsistencies are avoided becuse they require a lot of extra code logic that is, in my opinion, not worth the supposed flexibility.
...
Eventually, a set of features that belong together do so, because they share attributes, such as a group of islands that form a nation.
To capture attributes of sets or groups of features, a `geom` has an additional attribute tables, one that captures those group attributes.
...
It becomes clear, as mentioned above, that the this approach to attribute tables makes `geom`s quite flexible.
One could for example assign some measurements of a point pattern (such as [Arne Pommerening's](http://www.pommerening.org) Clocaenog 6 sample data) to the attribute table of a point `geom`.
...
```{r}
treeTheme <- setTheme(geom = list(pointsize = c(0.05, 0.77), pointsymbol = 21))
# visualise(`Clocaenog - tree diameters` = trees, pointsize = "dbh", theme = treeTheme)

treeTheme <- setTheme(geom = list(pointsize = c(0.05, 0.77), pointsymbol = 21))
# visualise(`Clocaenog - tree diameters` = trees, pointsize = "dbh", linecol = "height", theme = treeTheme)
```
