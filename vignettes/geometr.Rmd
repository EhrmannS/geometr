...
visualise(!!title := gt_subset(trees, deciduous), linecol = height)
```
...
(somePoints <- gs_point(anchor = aPoly) %>%
  gt_subset(!(duplicated(x) & duplicated(y))))
...
```{r}
treeTheme <- setTheme(parameters = list(linecol = c("#00204DFF", "#73D216FF", "#FFEA46FF", "#CC0000F0")))
