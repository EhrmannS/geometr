...
The `geometr` generalises the way to interact with spatial and other geometric objects by providing functions that access and modify data components in the same manner across classes.
...
This vignette outlines in detail first how `geometr` improves interoperability, then it describes the datastructure of a `geom`, how different feature types are cast into one another, shows how to visualise geometric objects and eventually gives a short introduction of the tools that come with `geometr`.
...
For a human user the structure of those information might not matter because we recognise, in most cases intuitively, where which information is to be found in such a simple datastructure.
...
| `point` | `tibble` | the coordinates in x and y dimension and the ID of the feature the point is part of (`fid`). Any other attributes that are valid for each individual point can be joined to this table. |
...
| `window` | `tibble` | the coordinates of a rectangular polygon that outlines the \"enclosing area\" of the `geom`. This is not to be confused with the extent, which is the minimum and maximum values of all dimentions of the `geom` and which is not recorded in a slot but derived from the coordinates. |
...
For raster objects it is intuitively clear that attributes are contained in seperate tables, where unique values of the raster cells are related to attribute values. 
...
In `geometr`, in contrast, a feature is a single point, one sequence of points that form a line or a [single polygon](#how-are-polygons-handled), i.e., only a single object of each tpye.
...
1. each feature can have both, ancillary information at the feature leve and at the group level (i.e., a feature in the \"simple feature\" sense).
...
All paths that are supposed to be nested within this paths must themselves be closed, but their order does not matter.
...
One could for example assign some point specific measurements (such as [Arne Pommerening's](http://www.pommerening.org) Clocaenog 6 sample data) to the point attribute table, and at the same time set group specific attributes to the group attribute table of a point `geom`.
...
trees <- locations %>%
  gs_point(anchor = .) %>%
  setPoints(table = prop) %>%
  setGroups(table = species)  %>%
  getPoints(!is.na(height))
